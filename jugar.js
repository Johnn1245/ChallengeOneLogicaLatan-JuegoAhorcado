...
let vectorPalabras = ["MURCIELAGO", "PANTERA", "OTORONGO", "JIRAFA", "COCODRILO", "MONO", "TIGRE", "HIPOPOTAMO", "PERRO", "LEOPARDO", "GORRILA", "BUFALO", "GAVIOTA", "ELEFANTE", "LORO", "ARDILLA"];
let palabra = [];
let palabraAux = [];
...
function palabraAlAzar() {
   ...
   let p = vectorPalabras[x];
   let palabra = Array.from(p);
   return palabra;
}
      ...
      if (!letrasIngresadas.includes(letra)) {
         ...
         if (palabraAux.includes(letra)) {
            ...
            if (errores == maximoErrores) {
               ...
               setTimeout(dibujarSolucion, 500);
